...
void USART1_IRQHandler(void)
{
  ...
  if ((__HAL_UART_GET_FLAG(&hUartDbg, UART_FLAG_RXNE) != RESET) && (__HAL_UART_GET_IT_SOURCE(&hUartDbg, UART_IT_RXNE) != RESET))
  {
	InputQueue(&dbg_in_q, (uint8_t)(hUartDbg.Instance->DR));
	return;
  }
...
void USART3_IRQHandler(void)
{
  ...
  {
	MB_readByte((uint8_t)(huart3.Instance->DR));
